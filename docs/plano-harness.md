...
Uma skill só roda quando o modelo decide invocá-la ou quando você digita `/nome`. Isso a torna
...
- Ponteiros: `docs/arquitetura-tecnica.md`, `docs/adr/`, `docs/produto/`, PRD
...
### 3.2 `/modulo` — criar módulo com documentação de nascença
...
### 3.3 `/conformidade` — revisar diff contra os invariantes
...
### 3.4 `/produto` — sincronizar a documentação funcional
...
`docs/produto/` é o documento **vivo**: o que existe de fato, hoje.

O que faz, para o sistema indicado:
1. Cria ou atualiza `docs/produto/<sistema>.md` com o comportamento implementado
...
| `Stop` | se a sessão tocou `sim/` ou `protocol/` sem rodar `/conformidade`, lembra |
...
- **`docs-check`**: todo pacote tem `CLAUDE.md`; todo ADR está no índice; a numeração de ADR não
  ...
  Além disso, lista as pendências `[ABERTO]` de `docs/produto/` como **inventário informativo**,
...
docs/
  arquitetura.md                   restrições do motor
  arquitetura-tecnica.md           arquitetura e plano do MVP
  plano-harness.md                 este documento
  adr/
    README.md                      índice
    0001-....md
  produto/
    README.md                      índice de sistemas
    hunt.md  bot.md  economia.md   documentação funcional viva
packages/
...
**Fazer antes do E0, não depois.** A skill `/modulo` é o que vai criar os pacotes do E0 já com
...
| **2** | Skills `/adr` e `/modulo` · **backfill dos 10 ADRs já decididos** | 1 dia |
| **3** | Skill `/conformidade` · lint de fronteiras · CI `docs-check` | 1 dia |
| **4** | Skills `/produto` e `/entregar` · templates de `docs/produto/` | meio dia |
...
- **Skill de "revisar código" genérica.** `/code-review` já existe no Claude Code e é melhor.
  `/conformidade` é específica dos invariantes deste projeto, que é onde ela agrega.
