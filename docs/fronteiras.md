...
**Pureza — I/O, rede, banco, framework, relógio global.** Esta é a regra que `docs/arquitetura.md`
...
2. **Custo.** A mesma hunt roda a 10 Hz anexada e a 1–2 Hz desanexada com resultado idêntico
   ...
   (`docs/arquitetura-tecnica.md` §14) ser viável.
3. **Opcionalidade de linguagem.** Se o custo de CPU virar gargalo, este núcleo — e só ele —
   ...
   (`docs/arquitetura.md`, seção de stack do servidor). Isso só continua verdade enquanto `sim`
...
sintético de carga (`docs/arquitetura-tecnica.md` §17, épico E0). Por isso ele pode importar os
...
(`"@algum-escopo/sim"`) e um import relativo cru que atravesse pacotes
