...
estão no [ADR 0011](adr/0011-library-stack.md); aqui é só onde as coisas rodam.
...
[ADR 0001](adr/0001-session-decoupled-from-connection.md) em silêncio**. A hunt deixa de ser a mesma
...
12 MB/s × 2,6 milhões from segundos/mês ≈  31 TB/mês from egresso
```
...
nenhuma** ([ADR 0003](adr/0003-variable-session-tick-rate.md)), e o protocolo é por evento com
...
2. Configuração **só** por variável de ambiente — já é o caso, e `loadConfiguration` derruba
...
Se ficar Supabase Auth em vez de WorkOS, o [ADR 0012](adr/0012-workos-delegated-authentication.md)
precisa ser revisto — a decisão não muda de forma, só de fornecedor: `account.external_auth_id`
continua sendo o vínculo, e `password_hash` continua nulável e reservado. É uma troca de integração,
