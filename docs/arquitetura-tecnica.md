...
**Versão:** 1.0
**Entrada:** `Draconya_PRD_Consolidado_v0.9.md` + `arquitetura.md`
**Escopo:** responde aos 16 itens do §45 do PRD e quebra o MVP em épicos e tarefas.

> Onde este documento contraria `arquitetura.md`, ele prevalece — o PRD trouxe decisões
...
E uma que aumenta o custo: §7.1 permite **personagens ilimitados por conta com 2 ativos simultâneos**, os dois podendo estar em hunt AFK. O teto de simulação é `2 × contas ativas`, não `jogadores conectados`.
...
```
Sessão
  id, tipo, nóId, versãoDeConteúdo, criadaEm
  ruleset            # Cidade | Hunt | Treino | Quest | Boss | GuildWar
  mundo              # tilemap + entidades
  participantes[]    # CharacterRuntime — dono exclusivo do estado quente
  visualizadores[]   # 0..n conexões, opcionais
  agregados          # analisador de sessão
  ledgerSeq          # sequência para idempotência econômica
```
...
A taxa de tick é uma função de `(tipo, tem visualizador?)`:
...
- `char:{id}:session → {sessionId, nodeId, tipo}` com TTL, renovado pelo nó dono
...
```
ledger(id, character_id, session_id, seq, tipo, delta, ref, criado_em)
...
**Teto de 5 ações por segundo por personagem.** Esse cooldown por categoria *é* o freio de reação que `arquitetura.md` deixou em aberto — não é preciso inventar outro. Se o combate ficar longo demais em Guild War, o parâmetro a mexer é este.
...
Mantém o desenho de `arquitetura.md`: WebSocket binário, frame `[chave uint32][flags][JSON([opcode, props])]`, flags para deflate e lote, pacote de protocolo compartilhado em TypeScript, validação de schema na entrada, deltas com ressincronização.
...
```
content/
  monstros/     stats, level recomendado, loot table, XP
  hunts/        rota, spawns, 4 dificuldades, composição por dificuldade
  itens/        atributos base fixos, requisitos, slots de imbuement
...
```
account(id, email, senha_hash, criado_em, coins)
character(id, account_id, nome, vocacao, level, xp, skills_json,
          gold, capacidade, premium_ate, stamina_ms, stamina_atualizada_em,
          estado, sessao_id)
item_instance(id, character_id | market_offer_id | loot_box_id,
              tipo, cargas, imbuements_json,
...
guild_war_team(id, guild_id, nome), guild_war_match(...), guild_war_score(...)
market_offer(id, item_instance_id | coins_qtd, vendedor_character_id, preco, estado)
ledger(id, character_id, session_id, seq, tipo, delta, ref, criado_em)
payment(id, account_id, provider, provider_event_id UNIQUE, valor, coins, estado)
session_snapshot(id, tipo, no_id, versao_conteudo, blob, atualizado_em)
```
...
O maior risco de custo não é técnico: é o teto de `2 × contas`. Stamina de 24 h com regeneração 1:1 significa que um personagem pode caçar metade do tempo — isso é o freio econômico real e precisa ser monitorado como métrica de infraestrutura, não só de game design.
