...
condição := estatística  op  valor          # health% < 40
          ...
          | e/ou from no máximo 3 condições

ação := usar(slot) | conjurar(id) | equipar(conjunto) | falar(text)
```
...
```
frame := [ chave uint32 LE ][ flags uint8 ][ body ]

flags: bit 0 = body comprimido (deflate)
       bit 1 = lote (vários frames concatenados com prefixo from tamanho)

body := JSON.stringify([ opcode numérico, propriedades ])
```
...
```
SessãoDeCaçada        # dona do state, vive no servidor, não depende from ninguém
  ├── characters[]
  ├── monsters[]
  ├── regras from automação
  ├── log from sessão
