...
describe.runIf(ready)('critério de saída do M13 (§44.4, ADR 0027)', () => {
  it('quatro vocações numa party compartilhada rendem XP conservada (não last-hit), a bolsa vende e divide, e o ledger tem uma linha por membro', async () => {
    ...
    // emenda 2026-09-24/25) quando os quatro atendem a elegibilidade do TFS/Canary — mas o
    ...
    // por 2 minutos também esbarram ocasionalmente no alcance/atividade. O TETO verdadeiro não é
    // `kills × 3` (a cota compartilhada): `xpByDamage` não tem limite por cabeça além do XP
    // INTEIRO do monstro (`floor(dano/total × xp)`, sem dividir por elegíveis) — quem causa
    // 100 % do dano num abate por rateio leva os 5 inteiros, não 3. `kills × 5` (a XP CHEIA do
    // rato, `content.monsters` acima) é o único teto que vale para as duas rotas ao mesmo tempo,
    // e não é um número exato: o id da sessão nasce de `randomUUID()` neste teste ponta a ponta,
    // e a semente do RNG de combate (`Rng.fromSeed(session.id)`) varia com ele a cada execução.
    expect(seen.aggregates.kills).toBeGreaterThan(0);
    expect(seen.aggregates.xpGained).toBeGreaterThan(0);
    expect(seen.aggregates.xpGained).toBeLessThanOrEqual(seen.aggregates.kills * 5);
    ...
    // §525 (ADR 0027 emenda 2026-09-24/25): a XP compartilhada do TFS/Canary é TUDO OU NADA por
    ...
    // O teto por cabeça é `kills × 5` (a XP CHEIA do rato), não `kills × 3` (a cota
    // compartilhada): `xpByDamage` pode dar o abate INTEIRO a quem causou 100 % do dano nele.
    ...
    for (const receipt of ofSession) {
      ...
      expect(receipt.aggregates.xpGained).toBeLessThanOrEqual(receipt.aggregates.kills * 5);
    }
