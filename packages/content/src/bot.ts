...
export function validateBotConfig(config: BotConfig, content: Content): string[] {
  ...
  for (const category of BOT_CATEGORIES) {
    config[category].forEach((rule, slot) => {
      ...
      // ADR 0035 d.10). O alvo de ataque continua sendo o monstro. A checagem ignora efeito
  ...
  config.sets.forEach((set, setIndex) => {
    set.slots.forEach((slot, slotIndex) => {
      ...
      if ((slot.target?.kind ?? 'self') !== 'self') {
        const effect = slot.do.kind === 'spell'
          ? content.spells.get(slot.do.spellId)?.effect
          : content.supplies.get(slot.do.supplyId)?.effect;
        const friendly = effect !== undefined
          && (effect.kind === 'heal' || effect.kind === 'mana')
          && effect.target === 'friend';
        if (effect !== undefined && !friendly) {
          problems.push(
            `${where}: esta ${slot.do.kind === 'spell' ? 'magia' : 'supply'} não pode ser `
              + 'usada em outro personagem',
          );
        }
      }
