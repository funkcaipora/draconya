...
| 4 | O cliente só manda intenção — nunca dano, posição resolvida, loot, XP ou resultado de transação | `packages/protocol/**`, mensagens direção cliente→servidor | campos como `damage`, `damage`, `loot`, `xp`, `xpGanho`, `result`, `resultado`, `posicaoResolvida` em tipo de mensagem cuja direção é cliente→servidor |
...
| 7 | A versão de conteúdo é fixada na sessão e não muda no meio dela | `server/` (código de sessão) | leitura de conteúdo "atual"/"mais recente" (`content.getLatest()`, `loadCurrentContent()`) dentro do laço de uma sessão já criada, em vez de usar a `contentVersion` capturada na criação da sessão |
| 8 | Todo personagem está sempre em exatamente um ESTADO, cidade inclusive — e um estado ATIVO é sempre exatamente uma sessão hospedada (ADR 0024) | `server/` | estado de personagem (posição, HP, etc.) mantido ou mutado fora de `CharacterRuntime`/do modelo de sessão; código que trata "cidade" como um caso especial fora do modelo de sessão; um personagem com DUAS sessões hospedadas. Repouso SEM sessão é legítimo — a Cidade não simula nada, e a coluna `characters.state` é a resposta |
| 9 | Estado QUENTE só é escrito pela sessão dona — nenhum outro processo toca o `CharacterRuntime` (ADR 0024) | `server/` | escrita em campos de `CharacterRuntime` (posição, HP, mana, XP, skills, inventário, buffs, delta de gold) a partir de código que não é o laço de tick/handler da própria sessão — outro processo, um job, outro nó. **A linha do Postgres NÃO é estado quente**: `jobs` e `api` a escrevem, serializados pela trava de linha e pela chave única do invariante 10 — isso é legítimo e não é achado |
...
await ledger.append({ characterId: id, sessionId, seq: nextSeq, type: "loot", delta: lootValue });
```
...
1. `packages/sim/src/hunt-loop.ts:14` — invariante 1 (sim/ é puro): importa `pg`, acesso a banco
   dentro from sim/.
2. `packages/protocol/src/messages.ts:30` — invariante 4 (cliente só manda intenção): message
