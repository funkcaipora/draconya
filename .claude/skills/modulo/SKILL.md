...
Para os seis pacotes já descritos em `docs/arquitetura-tecnica.md` (E0 — Fundação), as fronteiras
...
- **Escopo do pacote:** olhe `packages/*/package.json` já existentes e leia o campo `name`. Se
  ...
  `@draconya/<nome>` e trate isso como a convenção adotada a partir de agora — diga isso no
  resumo final.
...
- **Como testar:** `pnpm --filter @draconya/<nome> test`, mais uma frase sobre o que a suíte
...
novo pacote — nome, propósito de uma linha, link para `packages/<nome>/CLAUDE.md`.
